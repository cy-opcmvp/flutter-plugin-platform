...
class PlatformCore implements IPlatformCore {
  ...
  Future<void> initialize() async {
    ...
    _syncEngine = _testSyncEngine ?? CrossPlatformCore(_networkManager, stateManager: _stateManager);
  ...
  Future<void> _preserveStateForModeSwitch(OperationMode fromMode, OperationMode toMode) async {
  ...
  Future<void> _restoreStateForMode(OperationMode mode) async {
    ...
    final pluginStates = await _stateManager.loadState<Map<String, dynamic>>('mode_switch_plugin_states');
    if (pluginStates != null) {
      for (final entry in pluginStates.entries) {
        await _stateManager.savePluginState(entry.key, Map<String, dynamic>.from(entry.value));
      }
    }
    
    // Restore user preferences
    final preferences = await _stateManager.loadState('mode_switch_preferences');
    if (preferences != null) {
  ...
  PlatformInfo _createPlatformInfo() {
    ...
    return PlatformInfo(
      type: type,
      version: '1.0.0', // This would come from package info in real implementation
