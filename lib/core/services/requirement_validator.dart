...
class RequirementValidator {
  ...
  Future<ValidationResult> validatePluginRequirements(
  ...
  Future<ValidationResult> validatePackageRequirements(
  ...
  Future<void> _detectFileSystemCapabilities() async {
    try {
      ...
      try {
        final testFile = File(
          '${tempDir.path}/test_write_${DateTime.now().millisecondsSinceEpoch}',
        );
      ...
      _systemCapabilities['fileSystemExecute'] =
  ...
  Future<void> _detectNetworkCapabilities() async {
    ...
    _systemCapabilities['networkServer'] =
  ...
  /// Validate platform support
  Future<ValidationResult> _validatePlatformSupport(
    PluginManifest manifest,
  ) async {
    final errors = <String>[];
    final warnings = <String>[];
  ...
  /// Validate system requirements
  Future<ValidationResult> _validateSystemRequirements(
    PluginManifest manifest,
  ) async {
    final errors = <String>[];
    final warnings = <String>[];
    ...
    for (final feature
        in manifest.security.resourceLimits.maxMemoryMB > 0
            ? ['memory']
            : <String>[]) {
  ...
  /// Validate dependencies
  Future<ValidationResult> _validateDependencies(
    PluginManifest manifest,
  ) async {
    final errors = <String>[];
    final warnings = <String>[];
  ...
  /// Validate platform assets
  Future<ValidationResult> _validatePlatformAssets(
    PluginPackage package,
  ) async {
    final errors = <String>[];
    final warnings = <String>[];
    ...
    if (!package.supportsPlatform(currentPlatform)) {
      errors.add(
        'Plugin package does not include assets for current platform: $currentPlatform',
      );
    }
  ...
  /// Validate security signature
  Future<ValidationResult> _validateSecuritySignature(
    PluginPackage package,
  ) async {
    final errors = <String>[];
    final warnings = <String>[];
    ...
    if (package.manifest.security.requiresSignature &&
        package.signature.signature.isEmpty) {
