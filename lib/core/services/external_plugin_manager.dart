...
class ExternalPluginManager {
  ...
  Future<void> launchExternalPlugin(String pluginId) async {
    ...
    if (!stateManager.transitionTo(PluginState.loading)) {
      throw StateError('Cannot launch plugin $pluginId from state ${stateManager.currentState}');
    }
  ...
  Future<void> updateExternalPlugin(String pluginId, PluginPackage newPackage) async {
    _ensureInitialized();
  ...
  Future<void> downloadAndInstallPlugin(String pluginId) async {
    ...
    try {
      ...
      final packageData = await _downloadPluginPackage(registryEntry.downloadUrl);
  ...
  Future<void> updatePluginWithRollback(String pluginId, String newVersion) async {
    _ensureInitialized();
    ...
    try {
      ...
      if (registryEntry == null || registryEntry.version != newVersion) {
        throw StateError('Plugin $pluginId version $newVersion not found in registry');
      }
  ...
  PluginDescriptor _createDescriptorFromPackage(PluginPackage package) {
    return PluginDescriptor(
      ...
      entryPoint: package.manifest.configuration['entryPoint'] as String? ?? 'main',
    );
  }
  ...
  Future<Uint8List> _downloadPluginPackage(String downloadUrl) async {
    try {
      ...
      if (response.statusCode != 200) {
        throw Exception('Failed to download plugin: HTTP ${response.statusCode}');
      }
  ...
  Future<void> _resolveDependencies(PluginPackage package) async {
    for (final dependency in package.dependencies) {
      if (dependency.required && !_installedPackages.containsKey(dependency.id)) {
        ...
        if (depRegistryEntry == null) {
          if (dependency.downloadUrl != null) {
            ...
            try {
              final depPackageData = await _downloadPluginPackage(dependency.downloadUrl!);
              ...
              final depPackage = await _parsePluginPackage(depPackageData, tempRegistryEntry);
              
              // Recursively resolve dependencies
              await _resolveDependencies(depPackage);
              await installExternalPlugin(depPackage);
            } catch (e) {
              throw Exception('Failed to install required dependency ${dependency.id}: $e');
            }
          } else {
            throw Exception('Required dependency ${dependency.id} not found and no download URL provided');
          }
        } else {
