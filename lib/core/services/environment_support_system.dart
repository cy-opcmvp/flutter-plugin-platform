...
class EnvironmentSupportSystem {
  ...
  final Map<String, Map<PluginEnvironment, PluginEnvironmentConfig>> _pluginConfigs = {};
  ...
  void setEnvironmentConfig(PluginEnvironment environment, EnvironmentConfig config) {
    _environmentConfigs[environment] = config;
  }
  
  /// Get plugin environment configuration
  PluginEnvironmentConfig? getPluginEnvironmentConfig(String pluginId, PluginEnvironment environment) {
    return _pluginConfigs[pluginId]?[environment];
  }
  
  /// Set plugin environment configuration
  void setPluginEnvironmentConfig(String pluginId, PluginEnvironment environment, PluginEnvironmentConfig config) {
  ...
  Future<DeploymentValidationResult> validateDeployment(
    ...
    if (envConfig == null) {
      errors.add('No configuration found for environment: ${targetEnvironment.name}');
    ...
    final requirementResult = await _validateEnvironmentRequirements(package, envConfig);
    errors.addAll(requirementResult.errors);
    warnings.addAll(requirementResult.warnings);
    
    // Validate security requirements
    final securityResult = await _validateEnvironmentSecurity(package, envConfig);
    errors.addAll(securityResult.errors);
    warnings.addAll(securityResult.warnings);
    
    // Validate resource requirements
    final resourceResult = await _validateEnvironmentResources(package, envConfig);
    errors.addAll(resourceResult.errors);
    warnings.addAll(resourceResult.warnings);
    
    // Validate dependencies
    final dependencyResult = await _validateEnvironmentDependencies(package, envConfig);
  ...
  Map<String, dynamic> getEnvironmentPluginConfig(String pluginId) {
    final pluginConfig = getPluginEnvironmentConfig(pluginId, _currentEnvironment);
  ...
  Future<void> _detectEnvironment() async {
    ...
    if (platformEnv.containsKey('STAGING') ||
        platformEnv.containsKey('TEST')) {
  ...
  Future<void> _loadEnvironmentConfigurations() async {
    ...
    _environmentConfigs[PluginEnvironment.development] = _createDevelopmentConfig();
    _environmentConfigs[PluginEnvironment.staging] = _createStagingConfig();
    _environmentConfigs[PluginEnvironment.production] = _createProductionConfig();
  }
  ...
  Future<void> _applyEnvironmentConfiguration(PluginEnvironment environment) async {
  ...
  /// Notify about environment change
  Future<void> _notifyEnvironmentChange(PluginEnvironment oldEnvironment, PluginEnvironment newEnvironment) async {
    // In a real implementation, you would notify plugins and services about the environment change
    print('Environment changed from ${oldEnvironment.name} to ${newEnvironment.name}');
  }
  ...
  Future<DeploymentValidationResult> _validateEnvironmentRequirements(
    ...
    if (!envConfig.allowedPluginTypes.contains(package.type)) {
      errors.add('Plugin type ${package.type.name} not allowed in ${envConfig.name} environment');
    }
    
    // Check if external plugins are allowed
    if (!envConfig.allowExternalPlugins) {
      errors.add('External plugins not allowed in ${envConfig.name} environment');
    }
  ...
  Future<DeploymentValidationResult> _validateEnvironmentSecurity(
    ...
    if (package.manifest.security.level.index < envConfig.defaultSecurityLevel.index) {
  ...
  Future<DeploymentValidationResult> _validateEnvironmentResources(
    ...
    if (pluginLimits.maxMemoryMB > envLimits.maxMemoryMB) {
      errors.add('Plugin memory requirement (${pluginLimits.maxMemoryMB}MB) exceeds environment limit (${envLimits.maxMemoryMB}MB)');
    }
    
    if (pluginLimits.maxCpuPercent > envLimits.maxCpuPercent) {
      errors.add('Plugin CPU requirement (${pluginLimits.maxCpuPercent}%) exceeds environment limit (${envLimits.maxCpuPercent}%)');
    }
...
class EnvironmentConfig {
  ...
  factory EnvironmentConfig.fromJson(Map<String, dynamic> json) {
    return EnvironmentConfig(
      ...
      loggingLevel: LoggingLevel.values.firstWhere((e) => e.name == json['loggingLevel']),
      enabledFeatures: (json['enabledFeatures'] as List<dynamic>).cast<String>(),
      defaultSecurityLevel: SecurityLevel.values.firstWhere((e) => e.name == json['defaultSecurityLevel']),
      defaultResourceLimits: ResourceLimits.fromJson(json['defaultResourceLimits'] as Map<String, dynamic>),
      defaultPluginSettings: json['defaultPluginSettings'] as Map<String, dynamic>,
...
class PluginEnvironmentConfig {
  ...
  factory PluginEnvironmentConfig.fromJson(Map<String, dynamic> json) {
    return PluginEnvironmentConfig(
      ...
      environment: PluginEnvironment.values.firstWhere((e) => e.name == json['environment']),
      ...
      securityLevel: json['securityLevel'] != null
          ? SecurityLevel.values.firstWhere((e) => e.name == json['securityLevel'])
          : null,
      enabledFeatures: (json['enabledFeatures'] as List<dynamic>?)?.cast<String>() ?? const [],
    );
  }
