...
class IPCBridge {
  ...
  Future<void> _sendMessageWithRetry(String pluginId, IPCMessage message) async {
    ...
    for (int attempt = 0; attempt < retryConfig.maxAttempts; attempt++) {
      try {
        ...
        print('Message send attempt ${attempt + 1} failed for $pluginId: $e. Retrying in ${delay.inMilliseconds}ms');
        
        await Future.delayed(delay);
      }
    }
    
    throw lastException ?? Exception('Failed to send message after ${retryConfig.maxAttempts} attempts');
  }
  ...
  Future<void> broadcastEvent(PlatformEvent event) async {
    ...
    for (final entry in _connections.entries) {
      if (entry.value.isConnected) {
        futures.add(entry.value.sendMessage(message).catchError((error) {
          // Log error but don't fail the broadcast
          print('Failed to broadcast to ${entry.key}: $error');
        }));
      }
    }
  ...
  Future<void> _establishConnectionWithRetry(
    ...
    for (int attempt = 0; attempt < retryConfig.maxAttempts; attempt++) {
      try {
        ...
        // Calculate delay with exponential backoff
        final delay = _calculateBackoffDelay(attempt, retryConfig);
        print('Connection attempt ${attempt + 1} failed for $pluginId: $e. Retrying in ${delay.inMilliseconds}ms');
    ...
    _connectionAttempts.remove(pluginId);
    _retryConfigs.remove(pluginId);
    throw lastException ?? Exception('Failed to establish connection after ${retryConfig.maxAttempts} attempts');
  }
  ...
  Future<void> _handleMessage(String pluginId, IPCMessage message) async {
    try {
      ...
      if (message.messageType == 'response' || message.messageType == 'error') {
        final originalMessageId = message.messageId.replaceAll('_response', '').replaceAll('_error', '');
        ...
        if (completer != null && !completer.isCompleted) {
          if (message.messageType == 'error') {
            ...
            completer.completeError(IPCException(
      ...
      if (handler != null) {
        ...
        final errorResponse = message.createErrorResponse(
          ...
          errorMessage: 'No handler registered for message type: ${message.messageType}',
        );
        await sendMessage(pluginId, errorResponse);
      }
    } catch (e) {
      ...
      try {
        ...
        print('Failed to send error response to $pluginId: $sendError');
      }
    }
  }
  ...
  void _handleDisconnection(String pluginId) {
    print('Plugin $pluginId disconnected');
    
    // Check if auto-reconnect is enabled
    final retryConfig = _retryConfigs[pluginId];
    if (retryConfig?.autoReconnect == true) {
      print('Attempting to reconnect to plugin $pluginId');
  ...
  void _scheduleReconnection(String pluginId) {
    ...
    Timer(delay, () async {
      try {
        ...
        print('Reconnection failed for plugin $pluginId: $e');
  ...
  Future<void> _attemptReconnection(String pluginId) async {
    final connection = _connections[pluginId];
    if (connection == null) {
      throw StateError('No connection configuration found for plugin $pluginId');
    }
    ...
    print('Reconnecting to plugin $pluginId...');
    ...
    print('Reconnected to plugin $pluginId');
  }
  
  /// Handle connection errors
  void _handleConnectionError(String pluginId, dynamic error) {
    print('Connection error for plugin $pluginId: $error');
    
    // Check if this is a recoverable error
    if (_isRecoverableError(error)) {
      final retryConfig = _retryConfigs[pluginId];
      if (retryConfig?.autoReconnect == true) {
        print('Attempting recovery for plugin $pluginId');
        _scheduleReconnection(pluginId);
      }
    } else {
      print('Non-recoverable error for plugin $pluginId, closing connection');
  ...
  bool _isRecoverableError(dynamic error) {
    ...
    if (error is StateError && error.message.contains('connection')) return true;
  ...
  Duration _calculateBackoffDelay(int attempt, RetryConfig config) {
    ...
    final delayWithJitter = exponentialDelay + (exponentialDelay * config.jitterFactor * (DateTime.now().millisecond / 1000.0));
    
    // Cap at maximum delay
    final finalDelay = delayWithJitter.clamp(baseDelay.toDouble(), maxDelay.toDouble());
...
class NamedPipeConnection extends IPCConnection {
  ...
  Future<void> _sendWindowsPipeMessage(String messageJson) async {
    ...
    print('Windows pipe send: $messageJson');
  }
  ...
  Future<void> _sendUnixPipeMessage(String messageJson) async {
    ...
    await Future.delayed(const Duration(milliseconds: 10));
    print('Unix pipe send: $messageJson');
  }
...
class WebSocketConnection extends IPCConnection {
  ...
  Future<void> _simulateWebSocketSend(String messageJson) async {
    ...
    print('WebSocket send: $messageJson');
  }
...
class HTTPConnection extends IPCConnection {
  ...
  Future<void> _simulateHttpPost(Uri uri, String messageJson) async {
    // Simulate network delay
    await Future.delayed(const Duration(milliseconds: 100));
    print('HTTP POST to $uri: $messageJson');
  }
