...
class PluginSandbox {
  ...
  }) : _networkPolicy = networkPolicy ?? NetworkPolicy.fromSecurityLevel(securityLevel),
  ...
  Future<void> createSandbox() async {
    ...
    try {
      ...
      await _cleanup();
      throw SandboxException('Failed to create sandbox for plugin $pluginId: $e');
    }
  }
  ...
  Future<bool> validateAccess(AccessRequest request) async {
    ...
    try {
      ...
      if (!await _permissionManager.validateAccess(pluginId, request)) {
        ...
        _securityMonitor.logAccessViolation(pluginId, request.permission, request.resource, 'Permission denied');
        return false;
      }
      ...
      if (!await _withinResourceLimits(request)) {
        ...
        _securityMonitor.logAccessViolation(pluginId, request.permission, request.resource, 'Resource limit exceeded');
        return false;
      }
      ...
      if (!await _applySecurityPolicies(request)) {
        ...
        _securityMonitor.logPolicyViolation(pluginId, 'Security policy', 'Access to ${request.resource} denied by security policy');
        return false;
      }
      ...
      if (_isNetworkRequest(request) && !await _applyNetworkPolicies(request)) {
        ...
        _securityMonitor.logPolicyViolation(pluginId, 'Network policy', 'Network access to ${request.resource} denied by network policy');
        return false;
      }
  ...
  Future<ResourceUsage> getResourceUsage() async {
    if (!_isActive || _resourceMonitor == null) {
      throw SandboxException('Sandbox not active or resource monitor not initialized');
    }
  ...
  Future<void> enforceResourceLimits() async {
    ...
    try {
      ...
      if (usage.memoryMB > resourceLimits.maxMemoryMB) {
        _securityMonitor.logResourceViolation(pluginId, 'memory', usage.memoryMB, resourceLimits.maxMemoryMB.toDouble());
        throw SecurityViolationException('Memory limit exceeded: ${usage.memoryMB}MB > ${resourceLimits.maxMemoryMB}MB');
      }
      
      if (usage.cpuPercent > resourceLimits.maxCpuPercent) {
        _securityMonitor.logResourceViolation(pluginId, 'cpu', usage.cpuPercent, resourceLimits.maxCpuPercent);
        throw SecurityViolationException('CPU limit exceeded: ${usage.cpuPercent}% > ${resourceLimits.maxCpuPercent}%');
      }
      
      if (usage.networkKbps > resourceLimits.maxNetworkKbps) {
        _securityMonitor.logResourceViolation(pluginId, 'network', usage.networkKbps, resourceLimits.maxNetworkKbps.toDouble());
        throw SecurityViolationException('Network limit exceeded: ${usage.networkKbps}Kbps > ${resourceLimits.maxNetworkKbps}Kbps');
      }
      
      if (usage.fileHandles > resourceLimits.maxFileHandles) {
        _securityMonitor.logResourceViolation(pluginId, 'fileHandles', usage.fileHandles.toDouble(), resourceLimits.maxFileHandles.toDouble());
        throw SecurityViolationException('File handle limit exceeded: ${usage.fileHandles} > ${resourceLimits.maxFileHandles}');
      }
      
      if (usage.uptime > resourceLimits.maxExecutionTime) {
        _securityMonitor.logResourceViolation(pluginId, 'executionTime', usage.uptime.inSeconds.toDouble(), resourceLimits.maxExecutionTime.inSeconds.toDouble());
        throw SecurityViolationException('Execution time limit exceeded: ${usage.uptime} > ${resourceLimits.maxExecutionTime}');
      }
    } catch (e) {
      await terminate();
      rethrow;
  ...
  Future<void> terminate() async {
    ...
    try {
      ...
      throw SandboxException('Failed to terminate sandbox for plugin $pluginId: $e');
    }
  }
  ...
  Future<void> _configureNetworkPolicy() async {
    ...
    switch (securityLevel) {
      case SecurityLevel.isolated:
        ...
        break;
      case SecurityLevel.strict:
        _networkPolicy.allowOutbound = allowedPermissions.contains(Permission.networkClient);
        _networkPolicy.allowInbound = false;
        break;
      case SecurityLevel.standard:
        _networkPolicy.allowOutbound = allowedPermissions.contains(Permission.networkAccess) || 
                                      allowedPermissions.contains(Permission.networkClient);
        _networkPolicy.allowInbound = allowedPermissions.contains(Permission.networkServer);
        break;
      case SecurityLevel.minimal:
  ...
  bool _applyStrictPolicies(AccessRequest request) {
    ...
    return !restrictedResources.any((restricted) => resource.contains(restricted));
  }
  ...
  void _logSecurityViolation(String reason, AccessRequest request) {
    ...
    print('SECURITY_VIOLATION [$timestamp] Plugin: $pluginId, Reason: $reason, '
...
class ResourceUsage {
  ...
  Map<String, double> getUsagePercentages(ResourceLimits limits) {
    return {
      ...
      'executionTime': (uptime.inSeconds / limits.maxExecutionTime.inSeconds) * 100,
    };
  }
}
...
class NetworkPolicy {
  ...
  bool isAllowed(Uri uri) {
    ...
    if (blockedDomains.contains('*') || 
        blockedDomains.contains(uri.host)) {
      return false;
    }
