...
class PluginRegistryService {
  ...
  List<PluginRegistryEntry> searchPlugins(String query, {
    String? category,
    List<String>? tags,
    String? author,
    String? minVersion,
    ...
    if (category != null && category.isNotEmpty) {
      results = results.where((entry) => entry.category.toLowerCase() == category.toLowerCase()).toList();
    }
    ...
    if (minVersion != null) {
      results = results.where((entry) => _compareVersions(entry.version, minVersion) >= 0).toList();
    }
    if (maxVersion != null) {
      results = results.where((entry) => _compareVersions(entry.version, maxVersion) <= 0).toList();
    }
    ...
    if (securityLevel != null && securityLevel.isNotEmpty) {
      results = results.where((entry) => 
          entry.securityStatus.securityLevel == securityLevel).toList();
    }
    
    // Apply verification filter
    if (isVerified != null) {
      results = results.where((entry) => 
          entry.securityStatus.isVerified == isVerified).toList();
    }
  ...
  List<PluginRegistryEntry> getPluginsByCategory(String category) {
    return _registry.values.where((entry) => entry.category == category).toList();
  }
  ...
  List<PluginRegistryEntry> getPopularPlugins({int limit = 10}) {
    ...
    plugins.sort((a, b) => b.statistics.downloadCount.compareTo(a.statistics.downloadCount));
  ...
  List<PluginRegistryEntry> getTopRatedPlugins({int limit = 10}) {
    final plugins = _registry.values.toList();
    plugins.sort((a, b) => b.statistics.averageRating.compareTo(a.statistics.averageRating));
    return plugins.take(limit).toList();
  }
  ...
  PluginRegistryStatistics getRegistryStatistics() {
    ...
    return PluginRegistryStatistics(
      ...
      averageRating: ratedPluginCount > 0 ? totalRatingSum / ratedPluginCount : 0.0,
  ...
  List<PluginRegistryEntry> resolveDependencies(String pluginId) {
    ...
    void _resolveDependenciesRecursive(String currentPluginId) {
      ...
      if (visiting.contains(currentPluginId)) {
        throw StateError('Circular dependency detected involving plugin $currentPluginId');
      }
      ...
      if (currentPlugin == null) {
        throw ArgumentError('Dependency $currentPluginId not found in registry');
      }
  ...
  List<PluginRegistryEntry> getRecommendedPlugins({
    ...
    if (userCategories != null && userCategories.isNotEmpty) {
      candidates = candidates.where((plugin) => 
          userCategories.contains(plugin.category)).toList();
    }
    
    if (userTags != null && userTags.isNotEmpty) {
      candidates = candidates.where((plugin) => 
          plugin.tags.any((tag) => userTags.contains(tag))).toList();
    }
  ...
  List<PluginRegistryEntry> _findSimilarPlugins(
    ...
    for (final candidate in candidates) {
      ...
      final commonTags = candidate.tags.where((tag) => basePlugin.tags.contains(tag)).length;
      final totalTags = (candidate.tags.length + basePlugin.tags.length).toDouble();
      if (totalTags > 0) {
      ...
      final commonPlatforms = candidate.compatibility.supportedPlatforms
          .where((platform) => basePlugin.compatibility.supportedPlatforms.contains(platform))
          .length;
      final totalPlatforms = (candidate.compatibility.supportedPlatforms.length + 
                             basePlugin.compatibility.supportedPlatforms.length).toDouble();
      if (totalPlatforms > 0) {
  ...
  List<PluginRegistryEntry> _scoreAndSortPlugins(List<PluginRegistryEntry> plugins) {
    final scored = <_ScoredPlugin>[];
    ...
    for (final plugin in plugins) {
      ...
      final maxDownloads = plugins.map((p) => p.statistics.downloadCount).reduce((a, b) => a > b ? a : b);
      if (maxDownloads > 0) {
        score += (plugin.statistics.downloadCount / maxDownloads) * 0.2;
      }
      
      // Recency score (newer plugins get slight boost)
      final daysSinceUpdate = DateTime.now().difference(plugin.lastUpdated).inDays;
  ...
  List<PluginRegistryEntry> getTrendingPlugins({int limit = 10}) {
    ...
    plugins.sort((a, b) {
      ...
      final aScore = a.statistics.downloadCount * 
          (1.0 + (30 - DateTime.now().difference(a.lastUpdated).inDays.clamp(0, 30)) / 30.0);
      final bScore = b.statistics.downloadCount * 
          (1.0 + (30 - DateTime.now().difference(b.lastUpdated).inDays.clamp(0, 30)) / 30.0);
  ...
  List<PluginRegistryEntry> getFeaturedPlugins({int limit = 10}) {
    final featured = _registry.values
        .where((plugin) => plugin.tags.contains('featured') || 
  ...
  List<PluginRegistryEntry> advancedSearch(String query, {
    String? category,
    List<String>? tags,
    String? author,
    String? minVersion,
    ...
    results = results.where((plugin) => 
        ...
        plugin.statistics.downloadCount >= minDownloads).toList();
  ...
  void _sortPlugins(List<PluginRegistryEntry> plugins, SortBy sortBy, bool ascending) {
    switch (sortBy) {
      case SortBy.name:
        plugins.sort((a, b) => ascending 
      ...
      case SortBy.published:
        plugins.sort((a, b) => ascending 
            ? a.statistics.firstPublished.compareTo(b.statistics.firstPublished)
            : b.statistics.firstPublished.compareTo(a.statistics.firstPublished));
        break;
  ...
  List<PluginRegistryEntry> getPluginsBySubcategory(String category, String subcategory) {
...
class PluginRegistryEntry {
  ...
  bool isValid() {
    ...
    if (pluginId.isEmpty || name.isEmpty || version.isEmpty || 
        description.isEmpty || category.isEmpty || downloadUrl.isEmpty ||
        author.isEmpty || license.isEmpty) {
      return false;
    }
  ...
  factory PluginRegistryEntry.fromJson(Map<String, dynamic> json) {
    return PluginRegistryEntry(
      ...
      securityStatus: SecurityStatus.fromJson(json['securityStatus'] as Map<String, dynamic>),
      compatibility: CompatibilityInfo.fromJson(json['compatibility'] as Map<String, dynamic>),
      statistics: PluginStatistics.fromJson(json['statistics'] as Map<String, dynamic>),
...
class SecurityStatus {
  ...
  factory SecurityStatus.fromJson(Map<String, dynamic> json) {
    return SecurityStatus(
      ...
      securityWarnings: (json['securityWarnings'] as List<dynamic>).cast<String>(),
...
class PluginRegistryStatistics {
  ...
  factory PluginRegistryStatistics.fromJson(Map<String, dynamic> json) {
    return PluginRegistryStatistics(
      ...
      platformCounts: (json['platformCounts'] as Map<String, dynamic>).cast<String, int>(),
    );
  }
