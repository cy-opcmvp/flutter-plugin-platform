...
void main(List<String> args) async {
  ...
  p.commands['create-internal']!
    ..addOption('name', abbr: 'n', mandatory: true)
    ..addOption('type', abbr: 't', defaultsTo: 'tool')
    ..addOption('author', abbr: 'a')
    ..addOption('output', abbr: 'o');
  p.commands['create-external']!
    ..addOption('name', abbr: 'n', mandatory: true)
    ..addOption('language', abbr: 'l', defaultsTo: 'dart')
    ..addOption('author', abbr: 'a')
    ..addOption('output', abbr: 'o');
  p.commands['build']!
    ..addOption('plugin', abbr: 'p')
    ..addOption('platform', defaultsTo: 'current');
  p.commands['test']!
    ..addOption('plugin', abbr: 'p')
  ...
  p.commands['package']!
    ..addOption('plugin', abbr: 'p')
    ..addOption('output', abbr: 'o')
    ..addOption('platform', defaultsTo: 'all');
  p.commands['validate']!.addOption('plugin', abbr: 'p', mandatory: true);
  p.commands['publish']!
    ..addOption('plugin', abbr: 'p', mandatory: true)
    ..addOption('registry', abbr: 'r', defaultsTo: 'local');
  try {
...
Future<void> _run(ArgResults c) async {
  switch (c.name) {
    ...
    case 'create-external':
      ...
      break;
    case 'list-templates':
      print('Templates: internal(tool/game), external(dart/python/web)');
      break;
    case 'build':
      await _build(c);
      break;
...
Future<void> _createInt(ArgResults a) async {
  final n = a['name'] as String,
      t = a['type'] as String,
      au = a['author'] as String? ?? 'Dev',
      o = a['output'] as String? ?? 'lib/plugins';
  print('Creating internal plugin: $n');
  final id = 'com.example.${_snake(n)}',
      cls = _pascal(n),
      f = _snake(n),
      d = path.join(o, f);
  ...
  final pt = File(path.join(td, 'plugin-template.dart')),
      ft = File(path.join(td, 'factory-template.dart'));
  if (!await pt.exists() || !await ft.exists()) {
  ...
  final r = {
    ...
    '{{PLUGIN_CATEGORY}}': t == 'game' ? 'entertainment' : 'productivity',
    '{{PLUGIN_TAGS}}': t == 'game' ? "'game'" : "'tool'",
    '{{PLUGIN_ICON}}': t == 'game' ? 'games' : 'extension',
    '{{LICENSE}}': 'MIT',
    '{{CREATION_DATE}}': DateTime.now().toString().split(' ')[0],
  };
  var pc = await pt.readAsString(), fc = await ft.readAsString();
  r.forEach((k, v) {
    pc = pc.replaceAll(k, v);
    fc = fc.replaceAll(k, v);
  });
...
Future<void> _createExt(ArgResults a) async {
  final n = a['name'] as String,
      l = a['language'] as String,
      au = a['author'] as String? ?? 'Dev',
      o = a['output'] as String? ?? 'external_plugins';
  ...
  if (!await Directory(td).exists()) {
    print('Template not found: $td');
    exit(1);
  }
  final r = {
    '{{PLUGIN_NAME}}': n,
  ...
  await for (final e in Directory(td).list()) {
    if (e is File) {
      var c = await e.readAsString();
      r.forEach((k, v) {
        c = c.replaceAll(k, v);
      });
      await File(path.join(d, path.basename(e.path))).writeAsString(c);
    }
  }
  print('Created: $d');
}
  ...
  if (await mf.exists()) {
    ...
    final cp = pl == 'current'
        ? (Platform.isWindows
              ...
              : 'linux')
        : pl;
    final r = await Process.run('flutter', ['build', cp]);
    if (r.exitCode != 0) {
      print('Failed');
      exit(1);
    }
    ...
    print('No manifest.json or pubspec.yaml');
    exit(1);
  }
}
...
Future<void> _pkg(ArgResults a) async {
  ...
  final arc = Archive();
  arc.addFile(
    ArchiveFile('manifest.json', await mf.length(), await mf.readAsBytes()),
  );
  await for (final e in Directory(p).list(recursive: true)) {
    if (e is File) {
      final r = path.relative(e.path, from: p);
      if (!r.startsWith('.') && !r.contains('/.')) {
        arc.addFile(ArchiveFile(r, await e.length(), await e.readAsBytes()));
      }
    }
  }
  final h = sha256
      .convert(utf8.encode(arc.files.map((f) => f.name).join('\n')))
      .toString();
...
Future<void> _val(ArgResults a) async {
  ...
  try {
    final b = await File(p).readAsBytes(),
        arc = ZipDecoder().decodeBytes(b),
        ns = arc.files.map((f) => f.name).toSet();
    if (!ns.contains('manifest.json')) {
      errs.add('Missing manifest.json');
    } else {
      final mf = arc.files.firstWhere((f) => f.name == 'manifest.json');
      final m =
          jsonDecode(utf8.decode(mf.content as List<int>))
      ...
      for (final f in [
        ...
        if (!m.containsKey(f)) errs.add('Missing: $f');
      }
      print('  ${m['name']} v${m['version']} (${m['runtimeType']})');
    }
    if (!ns.contains('signature.txt')) warns.add('No signature');
    print(
      '  Files: ${arc.files.length}, Size: ${(b.length / 1024).toStringAsFixed(1)} KB',
    );
  } catch (e) {
    errs.add('Parse error: $e');
  }
  if (errs.isNotEmpty) {
    print('Errors:');
    for (final e in errs) {
...
Future<void> _pub(ArgResults a) async {
  ...
  final mf = arc.files.firstWhere((f) => f.name == 'manifest.json');
  final m =
      jsonDecode(utf8.decode(mf.content as List<int>)) as Map<String, dynamic>;
  final id = m['id'] as String, v = m['version'] as String;
  if (rg == 'local') {
    final h =
